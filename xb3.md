...
| X3-08 | 玻意耳定律 | A | done |
| X3-09 | 查理定律 | A | done |
| X3-10 | 理想气体状态方程 | A | done |
| X3-11 | p-V 图像 | A | done |
| X3-12 | 气体实验定律微观解释 | B | done |
| X3-13 | 热力学第一定律 | A | done |
| X3-14 | 热力学第二定律 | A | done |
| X3-15 | 能量守恒 | A | done |
| X3-16 | 热机与效率 | B | done |
| X3-17 | 晶体与非晶体 | B | done |
| X3-18 | 液体表面张力 | B | done |
...
<h4 id="molecular-theory">X3-01 分子动理论 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画把三个微观图景放在同一画面：左侧显示大量分子随机运动，中间显示悬浮颗粒被分子撞击产生布朗运动，右侧显示两分子距离改变时“斥力、平衡、引力、近似无力”的状态。

#### 5. 交互动画
<iframe src="anim/xb3/molecular-theory.html" width="100%" height="600" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：温度 `T`、分子数 `N`、分子间距 `r/r0`。实时变化：平均速率比例 `∝√T`、平均动能比例 `∝T`、碰撞频繁程度、分子力状态。

#### 6. 观察任务
- 把温度 `T` 调高：看分子运动是否更快，布朗颗粒轨迹是否更剧烈。
- 把分子数 `N` 调大：看容器内碰撞是否更密集。
- 拖动 `r/r0`：观察 `r<r0`、`r=r0`、`r>r0`、`r` 很大时分子力状态如何变化。
...
| 画面元素 | 物理量 | 公式或关系 | 含义 |
|---|---|---|---|
| 蓝色小点运动快慢 | 分子平均速率量级 | `v ∝ √T` | 温度越高，平均速率越大 |
| 读数 `Ek比例` | 平均动能 | `Ek(平均) ∝ T` | 温度是平均动能标志 |
| 蓝点数量 | 分子数 `N` | 大量分子统计 | `N` 越多，宏观量越稳定 |
| 红色大颗粒轨迹 | 布朗运动 | 分子撞击不平衡 | 间接反映分子热运动 |
| 两分子间距 | `r/r0` | 近斥、平衡、远引 | 分子力随距离变化 |

#### 10. 适用条件
- 分子动理论解释的是大量微观粒子的统计行为，不用于描述一个分子的确定轨迹。
- `Ek(平均) ∝ T` 中的 `T` 必须是热力学温度，单位为 K。
...
**例题 1**（教材改编，基础）
关于布朗运动，下列说法是否正确：布朗运动是液体分子的无规则运动。

**答案与解析**
不正确。布朗运动是悬浮在液体或气体中的微小颗粒的无规则运动。颗粒受到周围大量分子不平衡撞击而抖动，所以布朗运动能间接反映分子的无规则运动。

**方法提炼**
看到“布朗运动”，先问运动主体是谁：主体是颗粒，不是分子。
...
**例题 2**（高考常见判断）
一定量气体温度从 `300 K` 升高到 `600 K`。分子的平均动能变为原来的多少倍？

**答案与解析**
分子平均动能与热力学温度成正比：

```text
Ek2/Ek1 = T2/T1 = 600/300 = 2
```

平均动能变为原来的 `2` 倍。

**方法提炼**
比较平均动能必须用 K，不能用摄氏温度直接相除。
...
**例题 3**（易错纠偏）
两个分子间距小于平衡距离 `r0` 时，分子间合力表现为引力还是斥力？

**答案与解析**
表现为斥力。分子间既有引力又有斥力；当 `r<r0` 时斥力占主导，合力把分子推开；当 `r=r0` 时合力为 0；当 `r>r0` 且距离不太大时引力占主导。

**方法提炼**
分子力题先定位间距：`r<r0` 看作“压得太近”，合力斥；`r>r0` 看作“拉得偏远”，合力引。

来源：易错题改编

---

<h4 id="avogadro">X3-02 阿伏伽德罗常数 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画把天平上的质量、物质的量仪表和粒子计数器放在同一画面。拖动样品质量 `m` 或摩尔质量 `M`，`n=m/M` 和 `N=nNA` 会同时更新，让学生看到 `NA` 是“每摩尔粒子数”的转换比例。

#### 5. 交互动画
<iframe src="anim/xb3/avogadro.html" width="100%" height="560" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：样品质量 `m`、摩尔质量 `M`、显示比例。实时变化：物质的量 `n`、粒子数 `N`、每个小点代表的粒子数。

#### 6. 观察任务
- 固定 `M`，把 `m` 加倍：看 `n` 和 `N` 是否也加倍。
- 固定 `m`，把 `M` 调大：看同样质量对应的物质的量是否变小。
- 看粒子图：注意画面小点只是按比例显示，真实粒子数通常是 `10^23` 量级。
...
- `1 mol H2O` 含 `NA` 个水分子，但含 `3NA` 个原子。
- 粒子数 `N` 很大，通常用科学计数法表示。
...
**例题 1**（教材改编，基础）
`18 g` 水的物质的量是多少？含有多少个水分子？取 `NA=6.02×10^23 mol^-1`，水的摩尔质量 `M=18 g/mol`。

**答案与解析**

```text
n = m/M = 18/18 = 1 mol
N = nNA = 1×6.02×10^23 = 6.02×10^23 个
```

含有 `6.02×10^23` 个水分子。

**方法提炼**
先用质量求 mol，再用 `NA` 把 mol 换成个数。
...
**例题 2**（高考常见模型）
`2 mol CO2` 含有多少个 CO2 分子？含有多少个原子？
...
若取 `NA=6.02×10^23 mol^-1`，原子总数为 `3.612×10^24` 个。

**方法提炼**
分子数和原子数不是同一个数，要看化学式里的原子个数。
...
**例题 3**（易错纠偏）
某物质含有 `3.01×10^23` 个分子，则它的物质的量是多少？

**答案与解析**

```text
n = N/NA = (3.01×10^23)/(6.02×10^23) = 0.5 mol
```

物质的量为 `0.5 mol`。

**方法提炼**
从粒子数反推 mol 时，用除法 `n=N/NA`，不是乘以 `NA`。

来源：易错题改编

---

<h4 id="molecular-motion">X3-03 分子热运动 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画显示大量分子在容器中无规则运动，并用红色大颗粒表示悬浮颗粒的布朗运动。拖动温度后，蓝色分子的平均运动快慢、平均动能读数和红色颗粒轨迹都会同步变化。

#### 5. 交互动画
<iframe src="anim/xb3/molecular-motion.html" width="100%" height="520" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：温度 `T`。实时变化：平均速率比例 `∝√T`、平均动能比例 `∝T`、布朗运动强弱。

#### 6. 观察任务
- 把温度从 `300 K` 降到 `100 K`：看分子运动和布朗颗粒抖动是否减弱。
...
| 蓝色分子运动快慢 | 平均速率量级 | `v ∝ √T` | 温度升高，平均运动更快 |
| 读数 `平均动能` | `Ek(平均)` | `Ek ∝ T` | 温度是平均动能标志 |
...
**例题 1**（教材改编，基础）
为什么温度升高时，墨水在水中扩散通常更快？

**答案与解析**
温度升高，水分子和墨水分子的平均动能增大，无规则运动更剧烈，分子彼此进入对方的过程更快，所以扩散更快。

**方法提炼**
扩散题抓住“分子无规则运动”，温度题补上“平均动能变大”。
...
**例题 2**（高考常见模型）
某气体温度由 `300 K` 升高到 `1200 K`，分子平均动能变为原来的多少倍？平均速率量级约变为原来的多少倍？

**答案与解析**

```text
Ek2/Ek1 = T2/T1 = 1200/300 = 4
v2/v1 = √(T2/T1) = √4 = 2
```

平均动能变为原来的 `4` 倍，平均速率量级约变为原来的 `2` 倍。
...
**例题 3**（易错纠偏）
布朗运动越剧烈，是否说明悬浮颗粒就是分子？

**答案与解析**
不是。布朗运动的主体是悬浮颗粒，颗粒受到周围大量分子不平衡撞击而运动。颗粒越小、温度越高，布朗运动越明显，但它仍然不是分子本身的运动。

**方法提炼**
布朗运动题要写两层：看到的是颗粒，原因是分子撞击。

来源：易错题改编

---

<h4 id="molecular-force">X3-04 分子力 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画显示两分子距离 `r` 可调，并在右侧同步画出分子力 `F-r` 曲线和分子势能 `Ep-r` 曲线。拖动 `r` 时，分子受力箭头、合力状态和势能读数会同步移动。

#### 5. 交互动画
<iframe src="anim/xb3/molecular-force.html" width="100%" height="520" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：分子间距 `r/r0`。实时变化：合力表现（斥力、约为零、引力）、分子势能状态、曲线上的当前位置。

#### 6. 观察任务
- 把 `r` 拖到小于 `r0`：看箭头是否把两分子推开。
- 把 `r` 拖到 `r0`：看合力是否约为 0，势能是否最小。
- 把 `r` 拖到大于 `r0`：看箭头是否把两分子拉回。
...
分子势能在 `r=r0` 附近最小。无论把分子从 `r0` 压近还是拉远，都需要外界做功，分子势能都会增大。

#### 8. 概念理解
- 分子力不是“只有引力”或“只有斥力”，而是引力和斥力共同作用后的合力表现。
- `r0` 是平衡距离，不是“没有分子力成分”，而是引力与斥力大小相等。
- 分子势能看距离是否偏离 `r0`，偏离越明显，势能通常越高。
...
| 画面元素 | 物理量 | 关系 | 含义 |
|---|---|---|---|
| 距离滑块 | `r/r0` | 与平衡距离比较 | 决定合力表现 |
| 左侧分子箭头向外 | 斥力 | `r<r0` | 分子被推开 |
| 左侧分子箭头向内 | 引力 | `r>r0` | 分子被拉回 |
| 绿色状态 | `F≈0` | `r=r0` | 引力和斥力平衡 |
| 蓝色势能曲线 | `Ep` | `r0` 附近最小 | 偏离 `r0` 势能增大 |

#### 10. 适用条件
- 本知识点讨论两个分子间距离改变时的合力表现。
- `r0` 是分子合力为零的位置，不是分子间没有任何相互作用。
- 分子力只在分子尺度近距离明显，宏观距离不能直接套用。
- 分子势能判断一般围绕 `r0`：从 `r0` 压近或拉远，势能都增大。
- 对气体作理想气体近似时，通常忽略分子间相互作用势能。

#### 11. 应用题型
- 判断题：给出 `r` 与 `r0` 的关系，判断引力/斥力/合力方向。
...
#### 12. 解题思路
1. 先把分子间距与 `r0` 比较。
2. `r<r0` 判斥力主导，`r>r0` 判引力主导，`r=r0` 判合力为零。
3. 若问势能，先找是否靠近或远离势能最低点 `r0`。
...
| “分子势能在 r=0 最小” | 压得太近斥力强，势能高 | `r0` 附近势能最小 |
...
**例题 1**（教材改编，基础）
两个分子距离 `r=0.9r0`，分子间合力表现为引力还是斥力？

**答案与解析**
因为 `r<r0`，分子被压得比平衡距离更近，斥力大于引力，合力表现为斥力，方向使两分子相互远离。

**方法提炼**
分子力方向先比较 `r` 和 `r0`，小于 `r0` 就按“推开”判断。
...
**例题 2**（高考常见模型）
两个分子从 `r0` 缓慢拉远到 `1.5r0`，分子势能如何变化？

**答案与解析**
`r0` 附近分子势能最小。从 `r0` 拉远时，需要克服分子引力做功，分子势能增大。

**方法提炼**
势能题先找最低点 `r0`：离开 `r0`，势能通常增大。
...
**答案与解析**
压缩固体时，分子间平均距离变小。距离过小时斥力迅速增大，表现出强烈抗压作用，所以固体很难被压缩。

**方法提炼**
宏观“难压缩”对应微观“近距离斥力急剧增强”。

来源：易错题改编

---

<h4 id="internal-energy">X3-05 内能 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画把内能拆成两条能量柱：分子总动能随温度和物质的量变化，分子势能随分子间距偏离平衡距离而变化。拖动温度、物质的量和分子间距，能看到内能不是“温度一个量”能完全代表的。

#### 5. 交互动画
<iframe src="anim/xb3/internal-energy.html" width="100%" height="570" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：温度 `T`、物质的量 `n`、分子间距 `r/r0`。实时变化：分子总动能示意、分子势能示意、内能示意和理想气体近似提示。

#### 6. 观察任务
- 固定 `n` 和 `r/r0`，升高 `T`：看分子动能柱和内能是否增大。
- 固定 `T`，增大 `n`：看分子数更多时总内能是否增大。
- 把 `r/r0` 从 `1` 压近或拉远：看分子势能是否升高。
...
| 画面元素 | 物理量 | 关系 | 含义 |
|---|---|---|---|
| 温度滑块 | `T` | `Ek(平均) ∝ T` | 决定平均动能 |
| 物质的量滑块 | `n` | 分子总数 `∝ n` | 分子越多，总能量越大 |
| 红色能量柱 | 分子总动能 | `∝ nT` | 温度和数量共同影响 |
| 蓝色能量柱 | 分子势能 | 与 `r/r0` 有关 | 偏离平衡距离升高 |
...
**答案与解析**
不包括。内能是物体内部所有分子的热运动动能和分子势能的总和。物体整体运动的动能属于机械能，不属于内能。

**方法提炼**
看到“整体运动”，放到机械能；看到“分子热运动和相互作用”，放到内能。
...
**例题 2**（高考常见模型）
一定量理想气体从状态 A 变化到状态 B，温度不变。内能如何变化？

**答案与解析**
对一定量理想气体，内能只与温度有关。温度不变，所以内能不变：
...
**例题 3**（易错纠偏）
一小杯热水和一大桶温水相比，哪一个内能一定更大？

**答案与解析**
不能只看温度判断。热水温度高，分子平均动能大；但大桶温水物质的量可能远大于小杯热水，总分子数更多，总内能可能更大。要比较内能，需要同时看温度、质量和物态。
...
<h4 id="temperature-scale">X3-06 温度与温标 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画把摄氏温度计、热力学温度读数和分子运动放在同一画面。拖动摄氏温度 `t`，温度计液柱、`T=t+273.15`、平均动能比例和分子运动快慢同步变化。

#### 5. 交互动画
<iframe src="anim/xb3/temperature-scale.html" width="100%" height="560" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：摄氏温度 `t`、另一物体温度 `tB`。实时变化：热力学温度 `T`、平均动能比例、两个物体是否达到热平衡。

#### 6. 观察任务
- 把 `t` 从 `0℃` 调到 `100℃`：看 `T` 是否从约 `273 K` 变到约 `373 K`。
...
**例题 2**（高考常见模型）
气体温度由 `27℃` 升高到 `327℃`，分子平均动能变为原来的多少倍？
...
**例题 3**（易错纠偏）
两个物体达到热平衡时，它们的内能一定相等吗？

**答案与解析**
不一定。热平衡只说明两物体温度相同，不再发生净热传递。内能还与物质的量、物质种类、物态和体积等有关，所以温度相同不代表内能相同。

**方法提炼**
热平衡看温度，内能比较还要看“有多少物质”和“是什么状态”。
...
<h4 id="gas-state-parameters">X3-07 气体状态参量 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画展示封闭气缸中的气体。拖动体积 `V`、温度 `T` 和物质的量 `n`，活塞位置、分子数量/运动快慢、压强箭头和压力表读数会同步变化。

#### 5. 交互动画
<iframe src="anim/xb3/gas-state-parameters.html" width="100%" height="590" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：体积 `V`、温度 `T`、物质的量 `n`。实时变化：压强 `p=nRT/V`、`pV/T`、活塞位置、撞壁强弱。

#### 6. 观察任务
- 固定 `T` 和 `n`，减小 `V`：看 `p` 是否增大。
...
| 分子运动快慢 | `T` | `Ek∝T` | 温度越高撞壁越猛烈 |
| 分子点数量 | `n` | 分子总数 `∝n` | 充入气体使 n 增大 |
...
**例题 1**（教材改编，基础）
描述一团封闭气体的状态，通常需要哪些状态参量？

**答案与解析**
需要压强 `p`、体积 `V`、热力学温度 `T`。若气体不是封闭定量，还必须明确物质的量 `n`。封闭定量气体中 `n` 不变，常比较 `p、V、T` 的变化。

**方法提炼**
气体题先问“是否定量”，再列 `p、V、T、n`。
...
**例题 3**（易错纠偏）
某气体从 `27℃` 升高到 `127℃`，能否说温度变为原来的 `127/27` 倍？
...
<h4 id="boyle-law">X3-08 玻意耳定律 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画固定气体的温度 `T` 和物质的量 `n`，只允许拖动体积 `V`。活塞位置、压强箭头、压力表和右侧 `p-V` 双曲线上的点会同步变化，并显示 `pV` 是否保持常量。

#### 5. 交互动画
<iframe src="anim/xb3/boyle-law.html" width="100%" height="580" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：体积 `V`、等温温度 `T`。实时变化：压强 `p`、常量 `pV=nRT`、`p-V` 图像位置。

#### 6. 观察任务
- 固定温度，逐渐减小 `V`：看 `p` 是否按反比增大。
...
**例题 1**（教材改编，基础）
一定量气体在等温条件下，初态压强 `1.0×10^5 Pa`，体积 `2.0 L`。若体积压缩到 `1.0 L`，求末态压强。

**答案与解析**

```text
p1V1 = p2V2
p2 = p1V1/V2 = 1.0×10^5×2.0/1.0 = 2.0×10^5 Pa
```
...
**例题 2**（高考常见模型）
一定量气体等温膨胀，体积变为原来的 3 倍，压强变为原来的多少？
...
**例题 3**（易错纠偏）
气体被快速压缩，温度明显升高，还能直接用 `p1V1=p2V2` 吗？

**答案与解析**
不能直接用。玻意耳定律要求温度不变。快速压缩时温度明显升高，压强变化同时受体积减小和温度升高影响，应使用理想气体状态方程：
...
**方法提炼**
看到“快速压缩、温度升高”，立刻取消等温假设。

来源：易错题改编

---

<h4 id="charles-law">X3-09 查理定律 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画固定容器体积 `V`，拖动热力学温度 `T`。分子运动快慢、撞壁箭头、压力表和 `p-T` 图像上的点同步变化；同时显示摄氏温度，提醒比例计算不能直接用 ℃。

#### 5. 交互动画
<iframe src="anim/xb3/charles-law.html" width="100%" height="580" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：热力学温度 `T`、物质的量 `n`。实时变化：压强 `p`、常量 `p/T=nR/V`、摄氏温度读数、`p-T` 直线位置。

#### 6. 观察任务
- 固定 `n`，把 `T` 从 `300 K` 调到 `600 K`：看 `p` 是否变为 2 倍。
...
- 气体近似理想气体。
- 温度必须使用热力学温度 K。
- 若容器膨胀或漏气，不能直接用 `p/T=常量`。

#### 11. 应用题型
- 计算题：已知 `p1、T1、T2` 求 `p2`。
...
**例题 1**（教材改编，基础）
一定量气体在体积不变时，初态温度 `300 K`、压强 `1.0×10^5 Pa`。温度升到 `600 K`，求末态压强。

**答案与解析**

```text
p1/T1 = p2/T2
p2 = p1T2/T1 = 1.0×10^5×600/300 = 2.0×10^5 Pa
```
...
**例题 2**（高考常见模型）
一定量气体等容升温，从 `27℃` 升到 `127℃`。压强变为原来的多少倍？
...
**例题 3**（易错纠偏）
一个刚性容器中气体温度不变，但向其中充入同种气体，压强是否仍由查理定律判断？

**答案与解析**
不能直接用查理定律。查理定律要求一定量气体，充气使物质的量 `n` 增大。此时应看 `pV=nRT`，在 `V、T` 不变时，`p` 随 `n` 增大而增大。

**方法提炼**
等容不够，还要定量；`n` 变了，常量就变了。

来源：易错题改编

---

<h4 id="ideal-gas-equation">X3-10 理想气体状态方程 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画展示理想气体状态方程的四个变量。拖动 `V、T、n` 时，活塞、分子数量、分子运动快慢、压强和 `pV/T` 读数同步变化，让学生看到三条实验定律其实都来自 `pV=nRT` 的不同控制变量情形。

#### 5. 交互动画
<iframe src="anim/xb3/gas-state-parameters.html" width="100%" height="590" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：体积 `V`、温度 `T`、物质的量 `n`。实时变化：压强 `p=nRT/V`、`pV/T=nR`、活塞位置和分子运动。

#### 6. 观察任务
- 保持 `n` 不变，任意改变 `V` 和 `T`：看 `pV/T` 是否保持不变。
- 把 `n` 调大：看 `pV/T` 是否随 `nR` 增大。
- 固定 `T`：观察它退化为玻意耳定律 `pV=常量`。
- 固定 `V`：观察它退化为查理定律 `p/T=常量`。
...
**例题 1**（教材改编，基础）
一定量理想气体初态 `p1=1.0×10^5 Pa`、`V1=2.0 L`、`T1=300 K`。末态体积 `V2=1.0 L`、温度 `T2=450 K`，求末态压强。
...
**例题 2**（高考常见模型）
一定量理想气体从 `27℃` 升到 `127℃`，体积不变。压强变为原来的多少？
...
**例题 3**（易错纠偏）
某容器体积、温度不变，气体压强变为原来的 `0.8` 倍。气体物质的量如何变化？
...
<h4 id="pv-graph">X3-11 p-V 图像 <span class="status done">done</span></h4>
...
#### 5. 交互动画
<iframe src="anim/xb3/pv-graph.html" width="100%" height="600" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可操作参数：过程类型、初态体积 `V1`、末态体积 `V2`、初态压强 `p1`。实时变化：过程线、末态压强 `p2`、气体做功符号和面积。

#### 6. 观察任务
- 选等温：看图线是否是双曲线，`V` 增大时 `p` 是否减小。
- 选等压：看图线是否水平，面积是否是矩形。
- 选等容：看图线是否竖直，面积是否为 0。
- 把 `V2` 调小：看过程箭头向左时，气体对外做功是否为负。
...
**例题 2**（高考常见模型）
气体在压强 `2.0×10^5 Pa` 下等压膨胀，体积由 `1.0×10^-3 m³` 增加到 `3.0×10^-3 m³`。气体对外做功多少？
...
**例题 3**（易错纠偏）
同一条 p-V 图线从右向左走，气体对外做功是正还是负？

**答案与解析**
从右向左表示体积减小，气体被压缩，`dV<0`。按气体对外做功为正的约定：
...
<h4 id="gas-law-micro">X3-12 气体实验定律微观解释 <span class="status done">done</span></h4>
...
#### 5. 交互动画
<iframe src="anim/xb3/gas-law.html" width="100%" height="520" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>
...
| 画面元素 | 微观因素 | 宏观量 | 公式对应 |
|---|---|---|---|
| 活塞压缩 | 撞壁频率增大 | `V` 减小、`p` 增大 | `p∝1/V`（T、n 定） |
| 分子速度变快 | 撞击更猛烈 | `T` 增大、`p` 增大 | `p∝T`（V、n 定） |
| 分子点更多 | 撞壁分子数增多 | `n` 增大、`p` 增大 | `p∝n`（V、T 定） |
...
**答案与解析**
等温表示分子平均动能不变，撞击猛烈程度基本不变。压缩使体积减小，分子到器壁距离变短，单位时间撞击单位面积器壁的次数增多，所以压强增大。
...
**例题 2**（高考常见模型）
一定量气体体积不变，温度升高时，压强为什么增大？

**答案与解析**
体积不变时，容器空间不变。温度升高使分子平均动能增大，分子运动更快，每次撞击器壁时动量变化更大，同时撞壁也更频繁，所以压强增大。

**方法提炼**
等容升温重点说“撞击更猛烈”，可兼提频率增加。
...
**例题 3**（易错纠偏）
气体体积变为原来的一半，同时热力学温度也变为原来的一半，物质的量不变，压强如何变化？

**答案与解析**
由 `p=nRT/V`，`n` 不变：

```text
p2/p1 = (T2/T1)/(V2/V1) = (1/2)/(1/2) = 1
```

压强不变。体积减小使压强增大，温度降低使压强减小，两个影响正好抵消。

**方法提炼**
多个因素同时变，必须回到状态方程，不靠直觉。
...
<h4 id="first-law">X3-13 热力学第一定律 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画把气体系统画成一个“内能槽”。拖动吸热量 `Q` 和外界对系统做功 `W`，能量箭头和内能槽高度同步变化，并实时显示 `ΔU=Q+W` 的正负。

#### 5. 交互动画
<iframe src="anim/xb3/first-law.html" width="100%" height="560" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：热量 `Q`、外界对系统做功 `W`。实时变化：内能变化 `ΔU`、能量流向、状态判断。
...
| 读数 `ΔU` | 内能变化 | `ΔU=Q+W` | 两种能量交换的代数和 |
| 状态提示 | 增/减/不变 | 看 `Q+W` | 判断内能变化方向 |

#### 10. 适用条件
- 适用于封闭热力学系统的能量变化分析。
- 使用公式前必须统一符号约定。
- 本页 `W` 指外界对系统做功；若题目用“气体对外做功”记为 `Wout`，则 `ΔU=Q-Wout`。
...
3. 判断做功正负：压缩正，膨胀负。
4. 代入 `ΔU=Q+W`。
5. 根据 `ΔU` 正负判断温度/内能趋势；理想气体还可联系温度。
...
| “循环过程 ΔU 不一定为 0” | 回到初态内能相同 | 循环 `ΔU=0` |
| “绝热过程内能不变” | 绝热只表示 Q=0 | 还要看 W |

#### 14. 例题与训练

**例题 1**（教材改编，基础）
某系统吸收热量 `500 J`，外界对系统做功 `200 J`。系统内能如何变化？
...
**例题 2**（高考常见模型）
某气体吸收热量 `800 J`，同时对外做功 `300 J`。求气体内能变化。

**答案与解析**
本页公式中 `W` 是外界对系统做功。气体对外做功 `300 J`，所以外界对气体做功：
...
<h4 id="second-law">X3-14 热力学第二定律 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画展示热机从高温热库吸热 `QH`，一部分变为功 `W`，另一部分必须排给低温热库 `QC`。拖动高低温热库温度和目标效率，动画会显示卡诺效率上限，并判断这个效率是否违背第二定律。

#### 5. 交互动画
<iframe src="anim/xb3/second-law.html" width="100%" height="570" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：高温热库温度 `TH`、低温热库温度 `TC`、热机效率 `η`。实时变化：最大可能效率 `ηmax=1-TC/TH`、输出功 `W`、排热 `QC` 和可行性判断。

#### 6. 观察任务
- 固定 `TH`，升高 `TC`：看最大效率是否降低。
- 固定 `TC`，升高 `TH`：看最大效率是否升高。
- 把效率调到 100%：看是否被判为不可能。
- 把效率调到超过卡诺上限：看为什么能量守恒还不够，还要满足方向性限制。
...
- 计算题：用 `η=W/QH` 或 `ηmax=1-TC/TH` 判断效率。
- 概念题：区分第一定律和第二定律的作用。
...
**答案与解析**
不能。热量可以在外界做功的帮助下从低温物体传到高温物体，例如冰箱；但不能自发地从低温传到高温。

**方法提炼**
看清“自发”二字；非自发过程需要外界影响。
...
**例题 2**（高考常见模型）
某热机从高温热库吸收 `1000 J` 热量，对外做功 `350 J`。它向低温热库放出多少热量？效率是多少？
...
```text
QH = W + QC
QC = QH - W = 1000 - 350 = 650 J
η = W/QH = 350/1000 = 35%
```

**方法提炼**
热机吸热不会全部变成功，剩余部分排给低温热库。
...
**例题 3**（易错纠偏）
某热机工作在 `600 K` 和 `300 K` 两热库之间，效率可能达到 `60%` 吗？

**答案与解析**
最大理论效率：

```text
ηmax = 1 - TC/TH = 1 - 300/600 = 50%
```
...
<h4 id="energy-conservation-thermal">X3-15 能量守恒 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画把能量账本做成三条输出柱。拖动输入能量、有用输出比例和内能增加比例时，散失能量自动补足，始终满足总量守恒；若试图让输出超过输入，动画会提示账本不可能。

#### 5. 交互动画
<iframe src="anim/xb3/energy-conservation-thermal.html" width="100%" height="560" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：输入能量、有效输出比例、内能增加比例。实时变化：有用输出、内能增加、散失能量、守恒检查。
...
热力学第一定律 `ΔU=Q+W` 是能量守恒在热现象中的一种表达。
...
| 守恒提示 | 账本检查 | `Ein=Euse+ΔU+Eloss` | 总量守恒 |
...
**答案与解析**
电热水壶把电能主要转化为水和壶体的内能，使温度升高；同时也有一部分能量散失到空气中。
...
**例题 2**（高考常见模型）
某装置输入 `1000 J` 能量，输出有用机械能 `320 J`，系统内能增加 `180 J`。散失到环境中的能量是多少？

**答案与解析**

```text
Ein = Euse + ΔU + Eloss
Eloss = 1000 - 320 - 180 = 500 J
```

散失能量为 `500 J`。

**方法提炼**
账本题按“输入 = 输出 + 储存 + 散失”逐项列。
...
**例题 3**（易错纠偏）
摩擦使滑块最终停下，滑块原来的动能是否消失了？

**答案与解析**
没有消失。滑块的机械能减少，主要转化为滑块、接触面和周围环境的内能。机械能不守恒，但总能量守恒。

**方法提炼**
看到摩擦，就把“机械能减少”翻译成“内能增加”。

来源：易错题改编

---

<h4 id="heat-engine-efficiency">X3-16 热机与效率 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画显示热机从高温热库吸热 `QH`，输出功 `W`，并向低温热库排热 `QC`。拖动效率时，`W` 和 `QC` 同步变化，并与卡诺效率上限比较。

#### 5. 交互动画
<iframe src="anim/xb3/second-law.html" width="100%" height="570" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：高温热库温度 `TH`、低温热库温度 `TC`、设定效率 `η`。实时变化：输出功 `W`、排热 `QC`、理论最大效率。

#### 6. 观察任务
- 固定 `QH=1000J`，提高效率：看 `W` 增大、`QC` 减小。
- 把效率调到 100%：看为什么 `QC=0` 被判为不可能。
- 改变 `TH` 和 `TC`：看热机效率上限如何变化。
- 对照公式：确认效率低不是能量消失，而是更多能量排给低温热库。
...
| 效率读数 | `η` | `η=W/QH` | 热量转功比例 |
| 上限读数 | `ηmax` | `1-TC/TH` | 可逆热机极限 |
...
2. 用 `QH=W+QC` 建账本。
3. 用 `η=W/QH` 求效率。
4. 若涉及理论上限，用 `ηmax=1-TC/TH`。
5. 比较实际效率与上限，判断是否可能。

#### 13. 易错点
| 易错说法 | 为什么错 | 改法 |
|---|---|---|
| “效率=W/QC” | 效率定义用吸热 QH 作分母 | `η=W/QH` |
| “排热是能量损失消失了” | 排热仍是能量 | 只是变成废热 |
| “热机可 100% 转化” | 违反第二定律 | 必须有 QC |
| “卡诺效率用摄氏温度” | 温度比必须用 K | 先换 K |
| “循环后内能一直增加” | 循环回到初态 | 一个循环 `ΔU=0` |

#### 14. 例题与训练

**例题 1**（教材改编，基础）
热机从高温热源吸收 `2000 J` 热量，对外做功 `600 J`。求效率和排热。

**答案与解析**

```text
η = W/QH = 600/2000 = 30%
QC = QH - W = 2000 - 600 = 1400 J
```
...
**例题 2**（高考常见模型）
某热机效率 `40%`，每个循环吸热 `1500 J`。每个循环对外做功多少？

**答案与解析**

```text
W = ηQH = 0.40×1500 = 600 J
```
...
**例题 3**（易错纠偏）
工作在 `500 K` 和 `300 K` 热库之间的热机，效率能否达到 `50%`？

**答案与解析**
最大理论效率：

```text
ηmax = 1 - TC/TH = 1 - 300/500 = 40%
```
...
<h4 id="crystal-amorphous">X3-17 晶体与非晶体 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画左侧显示晶体规则点阵和非晶体无规则排列，右侧显示加热曲线。切换材料后，晶体曲线有明显熔化平台，非晶体曲线连续上升并逐渐软化。

#### 5. 交互动画
<iframe src="anim/xb3/crystal-amorphous.html" width="100%" height="560" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可操作参数：材料类型、加热进度。实时变化：微观排列、温度曲线、熔点/软化区提示。

#### 6. 观察任务
- 选“晶体”：拖动加热进度，看温度是否出现平台。
- 选“非晶体”：拖动加热进度，看温度是否连续变化、逐渐软化。
...
**例题 1**（教材改编，基础）
某固体加热时温度先升高，到某温度后保持不变，完全熔化后温度继续升高。它更可能是晶体还是非晶体？

**答案与解析**
更可能是晶体。加热曲线中的温度平台表示该物质有固定熔点，熔化时吸热但温度不变，这是晶体的重要特征。
...
**例题 2**（高考常见判断）
玻璃在加热时逐渐软化，没有固定熔点。玻璃属于晶体还是非晶体？

**答案与解析**
玻璃属于非晶体。非晶体没有固定熔点，受热时逐渐软化。
...
**答案与解析**
不一定。单晶体内部排列方向统一，常表现为各向异性；多晶体由许多取向不同的小晶粒组成，整体效果可能相互抵消，常表现为各向同性。
...
<h4 id="surface-tension">X3-18 液体表面张力 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画显示液面表面分子受力不平衡、液面边界的张力方向和张力大小。拖动表面张力系数 `σ` 与边界长度 `L`，张力箭头和读数 `F=σL` 同步变化。

#### 5. 交互动画
<iframe src="anim/xb3/surface-tension.html" width="100%" height="560" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：表面张力系数 `σ`、边界长度 `L`、压在液面的物体重力 `G`。实时变化：表面张力 `F`、能否托住物体、液滴收缩提示。
...
| σ 滑块 | 表面张力系数 | `σ` | 液体种类和温度影响 |
| L 滑块 | 边界长度 | `L` | 受力边界越长总张力越大 |
| 读数 F | 总张力 | `F=σL` | 简单模型估算 |
| 物体重力 | `G` | 与 F 比较 | 判断能否被托住 |

#### 10. 适用条件
- `F=σL` 是简单边界模型，复杂曲面和接触角问题需更细分析。
...
**答案与解析**
液体表面张力使液体表面有收缩趋势。在体积一定时，球形表面积最小，因此小水滴趋向球形。
...
**例题 2**（高考常见模型）
某液体表面张力系数 `σ=0.07 N/m`，一段边界长度 `L=0.20 m`。估算表面张力大小。

**答案与解析**

```text
F = σL = 0.07×0.20 = 0.014 N
```
...
**例题 3**（易错纠偏）
细针能浮在水面上，是否说明针受到的浮力大于重力？

**答案与解析**
不一定。细针密度通常大于水，单靠浮力不足以解释。针使水面发生形变，表面张力沿弯曲液面产生向上的合效果，与浮力一起平衡重力。

**方法提炼**
细针浮水题要写“表面张力 + 液面形变”，不要只说浮力。
...
<h4 id="photoelectric-x3">X3-19 光电效应 <span class="status done">done</span></h4>
...
<svg viewBox="0 0 660 240" width="100%" style="max-width:840px">
  ...
  <text x="360" y="62" font-size="14" fill="#e03131" text-anchor="middle">光电子 Ek</text>
  ...
  <text x="533" y="128" font-size="18" fill="#102a43" text-anchor="middle">Ek = hν - W0</text>
...
可拖动参数：光频率 `ν`、逸出功 `W0`、光强。实时变化：截止频率 `ν0`、最大初动能 `Ek`、逸出电子数量。

#### 6. 观察任务
- 把 `ν` 调到低于 `ν0`：看是否没有电子逸出。
- 在 `ν<ν0` 时提高光强：看是否仍不能逸出电子。
- 把 `ν` 调高：看 `Ek=hν-W0` 是否增大。
...
电子逸出金属至少需要克服逸出功 `W0`，剩余能量转化为电子最大初动能：
...
| 画面元素 | 物理量 | 公式对应 | 含义 |
|---|---|---|---|
| 入射光颜色/频率 | `ν` | `E=hν` | 决定单个光子能量 |
| 金属材料滑块 | `W0` | 逸出功 | 电子离开金属的最低能量 |
| 截止频率读数 | `ν0` | `ν0=W0/h` | 低于它不逸出 |
| 电子速度/能量条 | `Ek` | `Ek=hν-W0` | 最大初动能 |
| 光强滑块 | 光子数密度 | 改变电子数量 | 不改变 `Ekmax` |

#### 10. 适用条件
- 方程中的 `ν` 是光频率，不是光强。
- `Ek` 是最大初动能，实际逸出电子动能可能更小。
- 低于截止频率时，增加光强不能使电子逸出。
...
- 计算题：由 `Ek=hν-W0` 求频率、逸出功或最大初动能。
- 图像题：读 `Ek-ν` 直线的斜率、截距和截止频率。
- 实验题：由遏止电压求最大初动能。
...
3. 若 `ν<ν0`，不发生光电效应。
4. 若 `ν≥ν0`，用 `Ek=hν-W0`。
5. 光强只放在“电子数/光电流”上，不放进 `Ek` 公式。

#### 13. 易错点
| 易错说法 | 为什么错 | 改法 |
|---|---|---|
| “光强足够大就能打出电子” | 单个光子能量不足仍不行 | 先看频率 |
| “光强越大 Ek 越大” | Ek 由频率决定 | 光强影响电子数 |
| “频率低时多照一会儿会积累能量” | 光子能量逐份吸收 | 不满足截止频率不逸出 |
| “所有金属截止频率相同” | W0 不同 | 材料决定 `ν0` |
| “Ek 可为负” | 负值表示不逸出 | 取不发生光电效应 |

#### 14. 例题与训练

**例题 1**（教材改编，基础）
某金属逸出功为 `2.0 eV`，入射光光子能量为 `3.5 eV`。光电子最大初动能是多少？

**答案与解析**

```text
Ek = hν - W0 = 3.5 - 2.0 = 1.5 eV
```

最大初动能为 `1.5 eV`。

**方法提炼**
光电效应能量账本：光子能量先付逸出功，剩下成为动能。
...
**例题 2**（高考常见模型）
某光频率低于金属截止频率。若把光强增大到原来的 100 倍，是否能发生光电效应？

**答案与解析**
不能。低于截止频率时，单个光子能量 `hν` 小于逸出功 `W0`，电子不能逸出。增大光强只增加光子数，不能提高单个光子的能量。
...
可知斜率为普朗克常量 `h`，横轴截距为截止频率 `ν0=W0/h`。

**方法提炼**
把图像和一次函数对应：斜率看 `h`，横截距看 `ν0`。

来源：易错题改编

---

<h4 id="wave-particle-duality">X3-20 波粒二象性 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画左侧显示光的波形和光子点列，右侧显示实物粒子的德布罗意波。拖动光频率、光强和粒子动量，能同时看到光子能量 `E=hν`、光子数量和物质波波长 `λ=h/p` 的变化。

#### 5. 交互动画
<iframe src="anim/xb3/wave-particle.html" width="100%" height="560" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：光频率 `ν`、光强、粒子动量 `p`。实时变化：光子能量、光子数量、德布罗意波长和波形疏密。
...
| 画面元素 | 物理量 | 公式对应 | 含义 |
|---|---|---|---|
| 光波疏密 | 频率 `ν` | `E=hν` | 频率越高光子能量越大 |
| 光子点数量 | 光强 | 光子数密度 | 光强越大光子越多 |
| 右侧物质波 | 波长 `λ` | `λ=h/p` | 动量越大波长越短 |
| 动量滑块 | `p` | `λ∝1/p` | 控制实物粒子波动性显著程度 |
| 读数面板 | `E、λ` | 量子关系 | 把现象和公式绑定 |
...
#### 11. 应用题型
- 概念题：判断哪些现象体现波动性或粒子性。
- 计算题：用 `E=hν` 或 `λ=h/p` 求能量、频率、波长、动量。
- 比较题：动量变化对德布罗意波长的影响。
- 易错题：区分光强和频率的作用。
...
| 易错说法 | 为什么错 | 改法 |
|---|---|---|
| “光强越大单个光子能量越大” | 单个光子能量由频率决定 | `E=hν` |
| “电子只有粒子性” | 电子可发生衍射 | 实物粒子也有波动性 |
| “宏观物体没有物质波” | 只是波长极小难观察 | `λ=h/p` 仍成立 |
| “波粒二象性是两种东西混在一起” | 是量子对象的统一属性 | 看实验表现 |
| “λ 与 p 成正比” | 德布罗意关系是反比 | `λ=h/p` |
...
**答案与解析**
光电效应体现光的粒子性。它说明光能量以光子为单位传递，单个光子的能量为 `E=hν`。
...
**例题 2**（高考常见模型）
某粒子动量变为原来的 2 倍，它的德布罗意波长变为原来的多少？
...
**例题 3**（易错纠偏）
同一种单色光，增大光强后，单个光子的能量是否增大？

**答案与解析**
不增大。单色光频率不变，单个光子能量 `E=hν` 不变。光强增大表示单位时间、单位面积通过的光子数增多。

**方法提炼**
频率管“每个光子多有能量”，光强管“有多少个光子”。

来源：易错题改编

---

<h4 id="atomic-structure">X3-21 原子结构 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画把金箔、原子核和 α 粒子轨迹放在同一画面。拖动入射位置、原子核电荷和电子云半径，可以看到：离原子核越近，库仑斥力越强，偏转越大；电子云范围很大，但真正造成大角散射的是很小的正电原子核。

#### 5. 交互动画
<iframe src="anim/xb3/atomic-structure.html" width="100%" height="560" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：碰撞参数 `b`、核电荷量等级 `Z`、电子云半径。实时变化：最近距离、库仑斥力强弱、偏转角和“直穿/偏转/反弹”判断。

#### 6. 观察任务
- 把 `b` 调大：看 α 粒子是否几乎直线穿过。
- 把 `b` 调小：看轨迹是否靠近原子核后发生大偏转。
...
| 画面元素 | 物理量 | 公式对应 | 含义 |
|---|---|---|---|
| α 粒子与原子核距离 | `r` | `F=kQq/r²` | 越靠近原子核，斥力越大 |
| 核电荷滑块 | `Q` 或 `Z` | `F∝Q` | 核电荷越大，同距斥力越强 |
| 入射位置滑块 | `b` | 最近距离改变 | `b` 小更容易大角偏转 |
| 电子云半径 | 原子半径量级 | `R原子 >> R核` | 原子大部分空间不是原子核 |
...
**例题 1**（教材改编，基础）
α 粒子散射实验中，为什么“大多数 α 粒子几乎不偏转”能说明原子内部大部分空间是空的？

**答案与解析**
如果原子内部充满均匀分布的正电物质，α 粒子经过时应普遍受到明显作用而偏转。实验中大多数几乎直穿，说明 α 粒子在大多数路径上没有遇到集中的正电荷和质量，原子内部大部分空间比较空旷。

**方法提炼**
“多数直穿”对应“空间空旷”，“少数大偏转”对应“小而集中的正核”。
...
**例题 2**（高考常见模型）
α 粒子靠近原子核时，受到的主要作用力方向如何？入射位置越靠近原子核，偏转一般怎样变化？

**答案与解析**
α 粒子和原子核都带正电，主要受到库仑斥力，方向沿 α 粒子与原子核连线并背离原子核。入射位置越靠近原子核，距离 `r` 越小，`F=kQq/r²` 越大，偏转一般越明显。

**方法提炼**
先判电性，再判力方向，最后用 `1/r²` 判断强弱。
...
**例题 3**（易错纠偏）
有人说：“α 粒子被大角反弹，说明原子核占据了原子的大部分体积。”这句话对吗？

**答案与解析**
不对。大角反弹说明原子核带正电、质量集中、作用很强，但反弹只发生在极少数 α 粒子上，恰恰说明原子核只占极小体积。大多数 α 粒子直穿，说明原子大部分空间不是原子核。

**方法提炼**
极少数大偏转说明“核很强”，多数直穿说明“核很小”。

来源：易错题改编

---

<h4 id="bohr-energy-level">X3-22 玻尔模型与能级 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画左侧显示玻尔轨道，右侧显示氢原子能级图。拖动初态和末态能级，可以看到电子从高能级落到低能级时发射光子，从低能级跃迁到高能级时吸收光子；光子的能量始终等于两能级的能量差。

#### 5. 交互动画
<iframe src="anim/xb3/bohr-energy-level.html" width="100%" height="580" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：初态能级 `ni`、末态能级 `nf`。实时变化：电子轨道位置、能级箭头方向、吸收/发射判断、能级差 `ΔE` 和对应光子频率。

#### 6. 观察任务
- 让 `ni>nf`：看电子向内层跃迁，是否发射光子。
- 让 `ni<nf`：看电子向外层跃迁，是否需要吸收光子。
- 改变两个能级的间隔：看 `ΔE` 越大，光子能量和频率是否越大。
...
| 画面元素 | 物理量 | 公式对应 | 含义 |
|---|---|---|---|
| 右侧水平线 | `En` | `En=-13.6/n² eV` | 每条线是一个允许能级 |
| 初态滑块 | `ni` | `Ei=-13.6/ni²` | 电子跃迁前所在能级 |
| 末态滑块 | `nf` | `Ef=-13.6/nf²` | 电子跃迁后所在能级 |
| 跃迁箭头 | `ΔE` | `ΔE=Ef-Ei` | 正值吸收，负值发射 |
| 光子读数 | `hν` | `hν=|ΔE|` | 光子能量等于能级差绝对值 |

#### 10. 适用条件
- 氢原子能级公式 `En=-13.6/n² eV` 主要用于氢原子或类氢离子的基础模型。
...
| “任意频率光都能被吸收” | 吸收光子能量必须等于能级差 | 先算 `ΔE` |
| “向外层跃迁会放光” | 向高能级需要吸收能量 | `Ef>Ei` 为吸收 |
| “发射光子能量等于末态能量” | 光子能量等于能级差 | `hν=|Ei-Ef|` |

#### 14. 例题与训练

**例题 1**（教材改编，基础）
氢原子从 `n=3` 跃迁到 `n=2`，是吸收光子还是发射光子？

**答案与解析**
`n=3` 能级高于 `n=2`，电子从高能级跃迁到低能级，原子能量减少，因此发射光子。
...
**例题 2**（高考常见模型）
氢原子能级 `E2=-3.40 eV`，`E1=-13.6 eV`。从 `n=2` 跃迁到 `n=1` 发射光子的能量是多少？
...
**例题 3**（易错纠偏）
某光子能量为 `2.00 eV`，氢原子某两个能级差为 `1.89 eV`。这个光子一定能被该跃迁吸收吗？

**答案与解析**
不能。原子吸收光子发生对应跃迁时，光子能量必须恰好等于两能级差。`2.00 eV` 与 `1.89 eV` 不相等，不能引起这个跃迁。
...
<h4 id="atomic-spectrum">X3-23 原子光谱 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画把“原子放光、棱镜分光、屏上出现谱线”连起来。拖动元素种类和激发强度，可以看到不同元素的谱线位置不同；激发强度主要改变亮度和可见线条数，不会把线状光谱变成连续彩带。

#### 5. 交互动画
<iframe src="anim/xb3/atomic-spectrum.html" width="100%" height="560" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：元素种类、激发强度、是否显示吸收光谱。实时变化：谱线位置、亮度、发射/吸收形式和对应波长读数。
...
| 画面元素 | 物理量 | 公式对应 | 含义 |
|---|---|---|---|
| 亮线或暗线位置 | 波长 `λ` | `c=λν` | 位置对应特定波长 |
| 谱线颜色 | 频率 `ν` | `ν=c/λ` | 颜色由频率决定 |
| 能级跃迁 | 能级差 `ΔE` | `hν=ΔE` | 一条谱线对应一次跃迁 |
...
**例题 1**（教材改编，基础）
为什么稀薄氢气放电后通过棱镜得到的是线状光谱，而不是连续光谱？

**答案与解析**
氢原子能级是离散的，电子只能在特定能级之间跃迁，发射光子的能量只能等于某些能级差。因此发出的光只含特定频率，经过分光后表现为几条固定位置的亮线。

**方法提炼**
线状光谱来自离散能级，连续光谱来自连续范围的光。
...
**例题 3**（易错纠偏）
提高放电管激发强度后，某元素原有谱线位置会不会因为“光更亮”而改变？

**答案与解析**
不会。谱线位置由原子的能级差决定，光更亮主要表示单位时间发出的光子数增多，或某些弱线更容易观察到，不表示同一跃迁的波长改变。
...
<h4 id="nuclear-structure">X3-24 原子核结构 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画把质子、中子、核素符号和中性原子的电子数放在同一个面板。拖动质子数和中子数，可以看到元素名称、质量数 `A`、中子数 `N`、核电荷数 `Z` 和核素符号同步变化。

#### 5. 交互动画
<iframe src="anim/xb3/nuclear-structure.html" width="100%" height="560" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：质子数 `Z`、中子数 `N`、电离状态。实时变化：质量数 `A`、核素符号、元素名称、电子数和净电荷。
...
**答案与解析**
左下角 `Z=6`，所以质子数为 `6`。左上角 `A=14`，所以中子数：
...
**答案与解析**
是。它们质子数都为 `6`，属于同一种元素；质量数不同，中子数分别为 `6` 和 `8`，因此互为同位素。
...
**例题 3**（易错纠偏）
某原子失去 2 个电子成为 `2+` 离子，它的质子数和中子数会改变吗？

**答案与解析**
不会。普通电离只改变核外电子数，不改变原子核内部的质子数和中子数。质子数仍决定元素种类，中子数仍决定同位素。
...
<h4 id="radioactive-decay">X3-25 放射性衰变 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画把母核、子核、放出粒子和核素符号放在一起。拖动衰变类型、母核质子数和中子数，可以看到 α 衰变、β⁻ 衰变、γ 衰变中 `A`、`Z` 的变化规则，并自动检查质量数和电荷数是否守恒。

#### 5. 交互动画
<iframe src="anim/xb3/radioactive-decay.html" width="100%" height="560" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：衰变类型、母核质子数 `Z`、母核中子数 `N`。实时变化：母核 `A/Z`、子核 `A'/Z'`、放出粒子、守恒检查和穿透/电离特点。
...
| α 粒子 | `⁴₂He` | `A' = A-4, Z'=Z-2` | 放出氦核 |
| β⁻ 粒子 | `⁰₋₁e` | `A'=A, Z'=Z+1` | 中子转化为质子并放出电子 |
| γ 光子 | `γ` | `A'=A, Z'=Z` | 核退激，不改核素 |
...
**例题 2**（高考常见模型）
某核素 `A_ZX` 发生一次 β⁻ 衰变，子核的 `A` 和 `Z` 如何变化？

**答案与解析**
β⁻ 衰变放出 `⁰₋₁e`，由守恒得质量数不变，核电荷数增加 1：
...
**例题 3**（易错纠偏）
γ 衰变后，原子核的质量数和核电荷数是否改变？

**答案与解析**
不改变。γ 射线是高频电磁波，表示原子核从较高能量状态退到较低能量状态并放出光子，核素的 `A` 和 `Z` 不变。
...
<h4 id="half-life">X3-26 半衰期 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画左侧显示一批原子核格点，右侧显示指数衰减曲线。拖动初始核数、半衰期和经过时间，可以看到剩余核数、已衰变核数和曲线点同步变化。

#### 5. 交互动画
<iframe src="anim/xb3/half-life.html" width="100%" height="560" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：初始核数 `N0`、半衰期 `T`、经过时间 `t`。实时变化：剩余核数 `N`、剩余比例、已衰变比例和图像位置。

#### 6. 观察任务
- 把时间调到 `T`：看剩余量是否约为 `N0/2`。
- 把时间调到 `2T`：看剩余量是否约为 `N0/4`。
- 改变 `T`：看半衰期越长，同样时间内衰变是否越少。
- 改变 `N0`：看剩余比例不变，但剩余核数随初始量改变。
...
| 画面元素 | 物理量 | 公式对应 | 含义 |
|---|---|---|---|
| 初始格点总数 | `N0` | 初始量 | 衰变前未衰变核数 |
| 时间滑块 | `t` | `n=t/T` | 已经过多少个半衰期 |
| 半衰期滑块 | `T` | `N=N0(1/2)^(t/T)` | 控制衰变快慢 |
| 亮色剩余格点 | `N` | 剩余量 | 未衰变核数 |
...
| “已衰变比例等于剩余比例” | 已衰变量是 `N0-N` | 分清剩余和衰变 |

#### 14. 例题与训练

**例题 1**（教材改编，基础）
某放射性样品初始有 `800` 个未衰变原子核，经过一个半衰期后约剩多少个？
...
**例题 2**（高考常见模型）
某核素半衰期为 `5 d`，初始质量为 `16 g`。经过 `15 d` 后还剩多少？
...
**例题 3**（易错纠偏）
把放射性样品加热后，半衰期会像化学反应速率一样明显变短吗？

**答案与解析**
一般不会。半衰期由原子核自身性质决定，普通温度、压强和化学状态通常不能显著改变核衰变快慢。
...
<h4 id="nuclear-reaction">X3-27 核反应 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画把入射粒子、靶核、产物核和放出粒子放在同一条核反应方程中。拖动反应模板，可以看到左右两边质量数总和和电荷数总和自动核对。
...
可拖动参数：反应模板、未知粒子位置。实时变化：方程、左右质量数、电荷数、未知粒子判断和反应类型说明。

#### 6. 观察任务
- 切换反应模板：看左右两边 `A` 总和是否相等。
- 再看左右两边 `Z` 总和是否相等。
- 把未知位置切到不同项：看如何由守恒反推未知粒子的 `A` 和 `Z`。
...
- 核反应会改变核素，不是普通化学反应。
- 配平方程时，质量数和电荷数必须分别守恒。
- 质子可写作 `¹₁H` 或 `¹₁p`，中子写作 `¹₀n`，电子写作 `⁰₋₁e`。
- 核反应中的能量变化来自核能，后续用质量亏损和结合能解释。

#### 9. 公式绑定
| 画面元素 | 物理量 | 公式对应 | 含义 |
|---|---|---|---|
| 每个核素左上角 | `A` | `ΣA左=ΣA右` | 质量数守恒 |
| 每个核素左下角 | `Z` | `ΣZ左=ΣZ右` | 电荷数守恒 |
...
| “中子电荷数是 1” | 中子不带电 | `¹₀n` |
| “电子质量数是 1” | β 电子质量数记 0 | `⁰₋₁e` |
| “质量数守恒说明没有能量变化” | 静止质量可亏损 | 后续用 `E=mc²` |

#### 14. 例题与训练

**例题 1**（教材改编，基础）
核反应 `¹⁴₇N + ⁴₂He → ¹⁷₈O + x` 中，`x` 是什么粒子？

**答案与解析**
质量数：左边 `14+4=18`，右边已知 `17`，所以 `x` 的质量数为 `1`。电荷数：左边 `7+2=9`，右边已知 `8`，所以 `x` 的电荷数为 `1`。因此 `x=¹₁H`，即质子。
...
**例题 2**（高考常见模型）
反应 `²₁H + ³₁H → ⁴₂He + ¹₀n` 是否满足质量数和电荷数守恒？

**答案与解析**
质量数：左边 `2+3=5`，右边 `4+1=5`。电荷数：左边 `1+1=2`，右边 `2+0=2`。两个都守恒。

**方法提炼**
核反应配平要做两张账：质量数账和电荷数账。
...
**例题 3**（易错纠偏）
有人说：“核反应方程质量数守恒，所以反应前后实际总质量一定完全相等。”这句话对吗？

**答案与解析**
不对。核反应方程中的质量数守恒表示核子数守恒，但反应前后静止质量可以有微小差别，差别对应核反应吸收或释放的能量。
...
<h4 id="nuclear-energy">X3-28 核能 <span class="status done">done</span></h4>
...
可拖动参数：质量亏损 `Δm`、反应次数、反应类型。实时变化：单次能量、总能量、质量转化比例和能量流向。
...
核反应中，若反应前后静止质量差为 `Δm=m前-m后`，且 `Δm>0`，则释放能量：
...
- 核能计算要注意单位：`kg` 配 `c²` 得焦耳，`u` 配 `931.5 MeV` 得兆电子伏。

#### 9. 公式绑定
| 画面元素 | 物理量 | 公式对应 | 含义 |
|---|---|---|---|
| 质量亏损滑块 | `Δm` | `E=Δmc²` | 决定单次释放能量 |
| 反应类型 | 裂变/聚变 | 核反应能 | 核结构改变释放能量 |
| 单次能量读数 | `E1` | `E1=Δm×931.5 MeV` | 用 `u` 计算核能 |
| 反应次数滑块 | `N` | `E总=N E1` | 大量反应能量累加 |
| 能量箭头 | 光、热、动能 | 能量转化 | 静止质量差转为其他能量 |

#### 10. 适用条件
- 用 `E=Δmc²` 时，`Δm` 必须是反应前后总静止质量差。
- 用 `931.5 MeV` 时，质量单位是 `u`。
- 释放能量要求 `m前>m后`；若 `m前<m后`，反应需要吸收能量。
...
#### 12. 解题思路
1. 找出反应前总静止质量 `m前` 和反应后总静止质量 `m后`。
2. 计算质量亏损 `Δm=m前-m后`。
3. 判断 `Δm` 正负，正值释放能量，负值吸收能量。
4. 若用 SI 单位，代入 `E=Δmc²`。
5. 若用原子质量单位，代入 `E=Δm×931.5 MeV`。
...
**例题 2**（高考常见模型）
核反应前后质量数守恒，是否说明反应前后静止质量完全相等？

**答案与解析**
不能。质量数守恒表示核子总数守恒，而原子核结合状态不同，反应前后总静止质量可以有微小差别，这个差别对应释放或吸收的能量。
...
**例题 3**（易错纠偏）
如果某核反应 `m前<m后`，它是释放能量还是吸收能量？

**答案与解析**
它需要吸收能量。释放能量要求反应后总静止质量小于反应前，即 `Δm=m前-m后>0`。
...
<h4 id="binding-energy-mass-defect">X3-29 结合能与质量亏损 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画把自由核子、结合后的原子核、质量亏损、结合能和平均结合能放在同一张账本中。拖动质量数、结合能和核子数，可以看到总结合能、平均结合能与稳定性提示同步变化。

#### 5. 交互动画
<iframe src="anim/xb3/binding-energy.html" width="100%" height="560" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：核子数 `A`、总结合能 `Eb`、对比核的平均结合能。实时变化：平均结合能、质量亏损、稳定性比较和能量账本。
...
| 原子核 | `m核` | 结合后质量 | 小于自由核子质量和 |
| 质量亏损读数 | `Δm` | `Δm=Zmp+Nmn-m核` | 对应释放或需要补回的质量差 |
| 结合能读数 | `Eb` | `Eb=Δmc²` | 拆核所需能量 |
...
**例题 1**（教材改编，基础）
某原子核质量亏损为 `0.020 u`，结合能约为多少？

**答案与解析**

```text
Eb = 0.020×931.5 MeV = 18.63 MeV
```

结合能约 `18.6 MeV`。

**方法提炼**
质量亏损用 `u`，乘 `931.5` 得 MeV。
...
**例题 2**（高考常见模型）
甲核总结合能 `80 MeV`、质量数 `10`；乙核总结合能 `90 MeV`、质量数 `15`。哪个平均结合能更大？
...
**例题 3**（易错纠偏）
有人说：“裂变放能是因为反应后总结合能变小。”这句话对吗？

**答案与解析**
不对。裂变产物通常具有更大的总结合能或更大的平均结合能趋势，反应后总静止质量减小，质量亏损对应释放能量。

**方法提炼**
放能看质量亏损；从结合能角度看，产物结合更紧。

来源：易错题改编

---

<h4 id="nuclear-fission">X3-30 核裂变 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画显示中子打中重核、重核裂成两个碎片并释放新中子。拖动每次裂变释放的中子数和控制棒吸收比例，可以看到链式反应是增长、稳定还是衰减。

#### 5. 交互动画
<iframe src="anim/xb3/nuclear-fission.html" width="100%" height="560" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：每次裂变释放中子数、控制棒吸收比例、代数。实时变化：每代有效中子数、裂变次数趋势和可控状态判断。

#### 6. 观察任务
- 提高每次释放中子数：看链式反应是否更容易增长。
- 提高控制棒吸收比例：看有效中子数是否减少。
...
**答案与解析**
一次裂变会放出新的中子，这些中子如果被其他可裂变核吸收，就能诱发下一代裂变，从而使反应一代一代传递下去。
...
**例题 2**（高考常见模型）
控制棒插入反应堆更深时，链式反应速率通常怎样变化？

**答案与解析**
控制棒吸收中子，插入更深会吸收更多中子，使有效中子数减少，链式反应速率降低。
...
**例题 3**（易错纠偏）
核电站和原子弹都涉及裂变，因此二者都要求链式反应迅速失控吗？

**答案与解析**
不对。核电站要求链式反应可控、稳定释放能量；原子弹要求极短时间内发生不可控的迅速链式反应。二者控制目标不同。
...
<h4 id="nuclear-fusion">X3-31 核聚变 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画显示两个带正电轻核从远处靠近。拖动温度和约束强度，可以看到热运动能量是否足以克服库仑斥力，并显示聚变概率和释放能量。

#### 5. 交互动画
<iframe src="anim/xb3/nuclear-fusion.html" width="100%" height="560" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：温度、约束强度、反应次数。实时变化：接近程度、聚变条件判断、单次能量和总能量。
...
**答案与解析**
质量数：左边 `2+3=5`，右边氦核为 `4`，所以 `x` 质量数为 `1`。电荷数：左边 `1+1=2`，右边氦核为 `2`，所以 `x` 电荷数为 `0`。因此 `x=¹₀n`，是中子。
...
**答案与解析**
轻核带正电，相互靠近时有库仑斥力。极高温使轻核具有足够大的热运动动能，才可能靠近到核力起作用的距离并发生聚变。
...
**例题 3**（易错纠偏）
核聚变和核裂变都能放能，它们的放能原因是否都可用质量亏损解释？

**答案与解析**
可以。二者反应对象不同：裂变是重核分裂，聚变是轻核结合；但只要反应后总静止质量小于反应前，质量亏损就对应释放能量。
...
<h4 id="particle-physics-intro">X3-32 粒子物理初步 <span class="status done">done</span></h4>
...
#### 4. 示意动画
动画把电子、质子、中子、夸克和中微子放在分类面板中。拖动粒子类型，可以看到它属于轻子、夸克还是强子，并显示电荷、是否复合以及典型参与的相互作用。

#### 5. 交互动画
<iframe src="anim/xb3/particle-physics.html" width="100%" height="560" frameborder="0" style="border:1px solid #d9e2ec;border-radius:8px"></iframe>

可拖动参数：粒子类型、反粒子开关、反应检查。实时变化：分类、电荷、组成、守恒提示和反粒子电荷关系。
...
**例题 2**（高考常见模型）
电子属于轻子还是强子？质子属于基本粒子吗？

**答案与解析**
电子属于轻子，目前看作基本粒子。质子属于强子，由夸克组成，不是基本粒子。
...
**例题 3**（易错纠偏）
反质子的电荷是多少？它是否因为叫“反”就没有质量？

**答案与解析**
反质子的电荷为 `-e`，与质子电荷相反。反粒子通常与对应粒子质量相同或相等，不是“没有质量”。

**方法提炼**
反粒子看电荷反号，不要把“反”理解为不存在质量。
