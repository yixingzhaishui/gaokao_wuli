...
| X3-31 | 核聚变 | A | done |
| X3-32 | 粒子物理初步 | C | done |
| X3-33 | 分子运动速率分布图像 | B | done |
| X3-34 | 扩散、布朗运动与温度微观解释 | B | done |
| X3-35 | 液晶、半导体与纳米材料 | C | done |
| X3-36 | 毛细现象 | B | done |
| X3-37 | 放射性同位素的应用 | B | done |
| X3-38 | 射线危害与防护 | B | done |
| X3-39 | 四种基本相互作用 | C | done |
...
可操作内容：直接拖动悬浮颗粒观察布朗运动轨迹，或拖动右侧分子改变分子间距 `r/r0`；也可调节温度 `T`、分子数 `N` 并记录数据。画面同时显示布朗运动俯视、分子力侧视、速率统计图像和数据记录，把平均速率 `∝√T`、平均动能 `∝T` 与可见运动绑定。

#### 6. 观察任务
- 把温度 `T` 调高：看分子运动是否更快，布朗颗粒轨迹是否更剧烈。
- 把分子数 `N` 调大：看容器内碰撞是否更密集。
- 拖动右侧分子改变 `r/r0`：观察 `r<r0`、`r=r0`、`r>r0`、`r` 很大时分子力状态如何变化。
...
可操作内容：直接拖动红色悬浮颗粒观察布朗轨迹，也可拖动温度计改变 `T` 并记录数据。画面同时显示显微镜俯视、温度计侧视、速率统计图像和数据记录，把平均速率 `∝√T`、平均动能 `∝T` 与可见运动绑定。
...
可操作内容：直接拖动分子改变间距 `r/r0`，也可播放热扰动下的微小振动并记录数据点。画面同时显示两分子俯视、`F-r` 图像、`Ep-r` 图像和数据记录，把“近斥、平衡零、稍远引、很远近似无力”绑定到箭头和曲线读数。

#### 6. 观察任务
- 直接拖动右侧分子到小于 `r0`：看箭头是否把两分子推开。
...
可操作内容：直接拖动两分子改变 `r/r0`，也可调节温度 `T`、物质的量 `n` 并记录能量数据。画面同时显示微观分子俯视、分子间距侧视、能量柱图像和数据记录，把 `U=分子总动能+分子势能` 与读数绑定。

#### 6. 观察任务
- 固定 `n` 和 `r/r0`，升高 `T`：看分子动能柱和内能是否增大。
- 固定 `T`，增大 `n`：看分子数更多时总内能是否增大。
- 直接拖动两分子，把 `r/r0` 从 `1` 压近或拉远：看分子势能是否升高。
...
可操作内容：直接拖动 A 或 B 的温度计液柱改变摄氏读数，也可播放热平衡过程并记录温标换算。画面同时显示双温度计侧视、分子热运动俯视、温标换算图像和数据记录，把 `T=t+273.15`、`ΔT=Δt` 与 K 读数绑定。
...
可操作内容：直接拖动等温气缸活塞改变体积 `V`，也可改变等温温度 `T` 并记录状态点。画面同时显示等温气缸侧视、压力表、`p-V` 图像和数据记录，把 `pV=常量`、`p1V1=p2V2` 与压力表读数绑定。
...
可操作内容：直接拖动温度计改变 `T`，也可调节物质的量 `n` 并记录状态点。画面同时显示等容容器侧视、温度计、压力表、`p-T` 图像和数据记录，把 `p/T=常量`、`p1/T1=p2/T2` 与 K 温标读数绑定。
...
#### 4. 示意动画
动画把气体系统画成一个可操作气缸。拖动活塞、热量手柄 `Q` 和做功手柄 `W`，气体装置、p-V 图像面积、能量账本和内能槽会同步变化，并实时显示 `ΔU=Q+W` 的正负。
...
可拖动参数：热量 `Q`、外界对系统做功 `W`、活塞位置 `V`；也可以直接拖动画面里的活塞和能量手柄。实时变化：内能变化 `ΔU`、能量流向、p-V 面积、压力/体积/温度读数、状态判断和数据记录。
...
可直接拖动：红色热库温度 `TH`、蓝色热库温度 `TC`、黑色效率阀 `η`；也可用右侧滑块精调。实时变化：热流箭头粗细、`W` 与 `QC` 读数、`ηmax=1-TC/TH` 图像游标、数据记录和可行性判断。

#### 6. 观察任务
- 先点“播放”：看 `QH` 进入热机后不是全部变成功，而是分成 `W` 和 `QC`。
- 拖蓝色低温热库并固定 `TH`：看 `TC` 升高时，最大效率 `ηmax` 是否降低。
- 拖红色高温热库并固定 `TC`：看 `TH` 升高时，最大效率 `ηmax` 是否升高。
...
可直接拖动：红色高温热库 `TH`、蓝色低温热库 `TC`、黑色效率阀 `η`。实时变化：输出功 `W=ηQH`、排热 `QC=QH-W`、理论最大效率、能流图像和数据记录。

#### 6. 观察任务
- 先点“播放”：看 `QH=1000J` 的热量包怎样分成黑色功流和蓝色废热流。
- 拖效率阀提高效率：看 `W` 增大、`QC` 减小，但不能越过 `ηmax`。
...
可拖动参数：表面张力系数 `σ`、边界长度 `L`、压在液面的物体重力 `G`；也可以直接拖动画面中的细针和边界尺。实时变化：表面张力 `F`、液面形变、张力计读数、能否托住物体和数据记录。
...
#### 4. 示意动画
动画左侧显示光子束和金属屏，右侧显示实物粒子双缝累积和德布罗意波。拖动频率探针、光强和动量探针，能同时看到光子能量 `E=hν`、光子数量、物质波波长 `λ=h/p` 和屏幕记录点的变化。
...
可拖动参数：光频率 `ν`、光强、粒子动量 `p`；也可以直接拖动画面里的频率探针和动量探针。实时变化：光子能量、光子数量、德布罗意波长、双缝累积点和仪表读数。
...
可操作内容：直接拖动入射 α 粒子改变碰撞参数 `b`，也可调节核电荷量等级 `Z`、电子云半径并记录偏转数据。画面同时显示 α 散射俯视、原子截面、偏转角图像和数据记录，把 `F∝Z/b²` 与“直穿/偏转/反弹”判断绑定。
...
可操作内容：直接拖动蓝色初态或红色末态手柄选择 `ni、nf`，也可播放跃迁过程并记录数据。画面同时显示轨道俯视、能级图像、光子波形和数据记录，把 `En=-13.6/n² eV`、`hν=|ΔE|` 与跃迁箭头绑定。
...
可操作内容：直接拖动谱线屏幕上的探针读取 `λ`，也可切换元素、激发强度和吸收/发射模式。画面同时显示光路侧视、谱线屏幕、能级图像和数据记录，把 `ν=c/λ`、`hν=hc/λ` 与谱线读数绑定。

#### 6. 观察任务
- 拖动谱线探针：看不同谱线对应的波长 `λ`、频率 `ν` 和光子能量 `hν`。
...
可拖动参数：初始核数 `N0`、半衰期 `T`、经过时间 `t`；也可以直接拖动画面中的时间游标。实时变化：剩余核数 `N`、剩余比例、已衰变比例、随机衰变点阵、图像位置和数据记录。
...
| 差值图像 | `ΔA、ΔZ` | `ΔA=0、ΔZ=0` | 两张账都归零才配平 |
...
可操作内容：直接拖动红色控制棒改变吸收比例，也可调节每次裂变释放的中子数和最大代数。画面同时显示反应堆俯视、控制棒侧视、代数图像和数据记录，把 `k=每次产生中子数×(1-吸收率)`、`E=Δmc²` 与链式反应状态绑定。
...
可操作内容：直接拖动氘核或氚核改变接近距离，也可调节温度、约束强度和反应次数。画面同时显示粒子轨迹俯视、约束侧视、能垒图像和数据记录，把 `²H+³H→⁴He+n+17.6 MeV`、`E=Δmc²` 与聚变概率绑定。
...
<h4 id="four-fundamental-interactions">X3-39 四种基本相互作用 <span class="status done">done</span></h4>
...
**解析**：天体具有质量，宏观尺度上引力长程累积效果明显，是行星轨道的主导因素。

**易错提醒**：天体通常近似电中性，不能把轨道运动归因于电磁吸引。

</details>

**例题 2：对象判断**（类型：基础计算）
原子中电子受到原子核吸引，主要是哪种相互作用？

<details><summary>点击查看答案与解析</summary>

**答案**：电磁相互作用。

**解析**：电子带负电，原子核带正电，二者之间的库仑吸引属于电磁相互作用。

**易错提醒**：原子尺度不一定都是核力，核力主要在原子核内部短程作用。

</details>

**例题 3：图像/实验**（类型：图像/实验）
在尺度图中把探针移到原子核内部，哪种相互作用变得关键？

<details><summary>点击查看答案与解析</summary>

**答案**：强相互作用。

**解析**：强相互作用在极短距离内很强，能束缚核子并克服质子间电磁斥力。

**易错提醒**：强相互作用不是长程力，离开核尺度很快不再主导。

</details>

**例题 4：综合模型**（类型：综合模型）
为什么原子核中多个质子没有仅因电磁斥力而立刻散开？

<details><summary>点击查看答案与解析</summary>

**答案**：原子核尺度内有强相互作用，它能在短距离内提供强束缚。

**解析**：质子间存在电磁斥力，但核子之间的强相互作用在核尺度内更强，使原子核能够稳定存在。
...
**例题 5：错解辨析**（类型：错解辨析）
某同学说：“强相互作用最强，所以它在宇宙大尺度也一定主导。”这个说法对吗？

<details><summary>点击查看答案与解析</summary>

**答案**：不对。

**解析**：强相互作用虽然在核尺度很强，但作用范围极短；宇宙大尺度中主要考虑长程引力。

**易错提醒**：比较相互作用要同时看强度和作用范围。

</details>

---

<h4 id="radiation-hazards-protection">X3-38 射线危害与防护 <span class="status done">done</span></h4>
...
**例题 1：射线比较**（类型：概念判断）
α、β、γ 三种射线中，通常穿透能力最强的是哪一种？

<details><summary>点击查看答案与解析</summary>

**答案**：γ 射线。

**解析**：γ 射线是高能电磁波，穿透能力强，常需要厚铅板或混凝土屏蔽。
...
**例题 2：剂量趋势**（类型：基础计算）
若只考虑距离影响，把到点源距离增大为原来的 2 倍，剂量率近似变为原来的多少？

<details><summary>点击查看答案与解析</summary>

**答案**：约为原来的 `1/4`。

**解析**：点源辐射强度近似按 `1/d²` 减小，距离加倍，剂量率约变为 `1/2²=1/4`。
...
**例题 3：屏蔽实验**（类型：图像/实验）
用纸、铝板、铅板分别屏蔽三种射线，哪种射线最需要厚铅板？

<details><summary>点击查看答案与解析</summary>

**答案**：γ 射线。

**解析**：α 射线穿透弱，β 射线可用薄铝板明显削弱，γ 射线穿透强，需要更厚更高密度材料屏蔽。
...
**例题 4：综合模型**（类型：综合模型）
医院放射检查中，工作人员为什么要站在屏蔽墙后并缩短停留时间？

<details><summary>点击查看答案与解析</summary>

**答案**：这是同时利用屏蔽和缩短时间来减少累计剂量。

**解析**：屏蔽材料削弱射线，缩短暴露时间减少总剂量；必要时还应增大距离。

**易错提醒**：防护不是单一措施，三原则常要组合使用。

</details>

**例题 5：错解辨析**（类型：错解辨析）
某同学说：“γ 射线穿透能力强，所以它的电离能力一定最强。”这个说法对吗？

<details><summary>点击查看答案与解析</summary>

**答案**：不对。

**解析**：穿透能力和电离能力不是同一指标。α 射线电离能力强但穿透弱，γ 射线穿透强但单位路径电离能力通常较弱。

**易错提醒**：比较射线要分别看穿透和电离两个维度。

</details>

---

<h4 id="radioisotope-applications">X3-37 放射性同位素的应用 <span class="status done">done</span></h4>
...
**例题 1：应用判断**（类型：概念判断）
用放射性同位素观察人体器官代谢，属于哪类应用？

<details><summary>点击查看答案与解析</summary>

**答案**：医学示踪。

**解析**：示踪剂随代谢进入相应器官，探测其发出的射线，可获得分布和功能信息。
...
**例题 2：半衰期计算**（类型：基础计算）
某核素半衰期为 `6 h`，经过 `12 h` 后剩余比例是多少？

<details><summary>点击查看答案与解析</summary>

**答案**：`1/4`。

**解析**：`12 h` 是两个半衰期，剩余比例为 `(1/2)^2=1/4`。

**易错提醒**：两个半衰期不是减去两个“一半”得到零。
...
**答案**：因为需要穿过金属工件，在探测器上形成内部缺陷造成的强度差。

**解析**：若射线穿透能力太弱，无法到达探测器，就难以显示内部结构。

**易错提醒**：穿透能力强也意味着防护要求更高。

</details>

**例题 4：综合模型**（类型：综合模型）
医学示踪剂为什么不宜选择半衰期特别长的核素？

<details><summary>点击查看答案与解析</summary>

**答案**：半衰期过长会使体内放射性停留时间过久，增加不必要剂量。

**解析**：医学示踪要在检查期间可探测，检查后尽快降低活度，所以半衰期要适中。

**易错提醒**：不是“越稳定越好”，要看应用时间尺度。

</details>

**例题 5：错解辨析**（类型：错解辨析）
某同学说：“放射性同位素只有危害，没有应用价值。”这个说法对吗？

<details><summary>点击查看答案与解析</summary>

**答案**：不对。

**解析**：放射性同位素在医学、工业、考古等领域有重要应用，但必须控制剂量、时间、距离和屏蔽。
...
<h4 id="capillary-phenomenon">X3-36 毛细现象 <span class="status done">done</span></h4>
...
**例题 1：现象判断**（类型：概念判断）
水能沿细玻璃管上升，说明水对玻璃通常表现为什么性质？

<details><summary>点击查看答案与解析</summary>

**答案**：浸润。

**解析**：水浸润玻璃时，液面在细管中形成凹液面，并在表面张力作用下出现毛细上升。

**易错提醒**：不是所有液体在所有管壁中都会上升。

</details>

**例题 2：管径影响**（类型：基础计算）
同种液体和管壁条件下，把毛细管半径减小到原来一半，毛细高度近似怎样变化？

<details><summary>点击查看答案与解析</summary>

**答案**：高度约变为原来的 2 倍。

**解析**：由 `h=2σcosθ/(ρgr)` 可知，在其他量不变时 `h∝1/r`。
...
**例题 3：图像实验**（类型：图像/实验）
在动画中把接触角从 `35°` 调到 `120°`，液柱会怎样变化？

<details><summary>点击查看答案与解析</summary>

**答案**：由上升变为下降。

**解析**：`35°<90°`，`cosθ>0`，液柱上升；`120°>90°`，`cosθ<0`，公式给出负高度，表示下降。

**易错提醒**：接触角跨过 `90°` 是判断方向的关键。
...
**答案**：纸巾内部有许多细小孔隙，水对纤维浸润，毛细作用使水沿孔隙上升和扩散。

**解析**：孔隙相当于许多细毛细管，半径小，毛细效应强，所以水能被吸入纸巾内部。

**易错提醒**：吸水不是纸巾“主动吸引”水，而是浸润和毛细作用共同结果。

</details>

**例题 5：错解辨析**（类型：错解辨析）
某同学说：“毛细管越粗，液体越容易上升得高。”这个说法对吗？

<details><summary>点击查看答案与解析</summary>

**答案**：不对。

**解析**：在其他条件相同时，毛细高度与半径近似成反比，管越细，升降越明显。

**易错提醒**：生活中“管粗流量大”和“毛细高度高”不是同一件事。

</details>

---

<h4 id="liquid-crystal-semiconductor-nanomaterial">X3-35 液晶、半导体与纳米材料 <span class="status done">done</span></h4>
...
**例题 1：液晶判断**（类型：概念判断）
液晶显示器能通过电压控制明暗，主要利用了什么？

<details><summary>点击查看答案与解析</summary>

**答案**：利用液晶分子取向受外电场影响，从而改变透光状态。

**解析**：电压改变液晶分子排列，配合偏振片可改变通过光的强弱。

**易错提醒**：液晶不是自己发光的所有来源，许多显示结构还需要背光。
...
**答案**：因为其载流子数量可受掺杂、温度、光照等因素显著影响。

**解析**：载流子越多，导电能力通常越强。半导体器件正是利用这种可控导电性工作。

**易错提醒**：半导体不是导电性固定不变的普通电阻。

</details>

**例题 3：图像/实验**（类型：图像/实验）
把纳米颗粒直径减小，动画中比表面积相对值怎样变化？

<details><summary>点击查看答案与解析</summary>

**答案**：增大。

**解析**：颗粒越小，相同体积材料暴露在表面的比例越大，表面效应更明显。

**易错提醒**：纳米材料的“特殊”常来自尺度效应，不是名字本身神秘。
...
**答案**：因为纳米颗粒比表面积大，能提供更多反应接触位置。

**解析**：相同质量下，尺度越小，表面原子比例越高，参与表面反应的有效位置越多。

**易错提醒**：催化效果不能简单理解成“质量越多越好”，还要看表面积和结构。

</details>

**例题 5：错解辨析**（类型：错解辨析）
某同学说：“半导体就是导电性很差的绝缘体。”这个说法对吗？

<details><summary>点击查看答案与解析</summary>

**答案**：不对。

**解析**：半导体导电性介于导体和绝缘体之间，关键是可调控，能通过掺杂、温度、光照等改变载流子数量。

**易错提醒**：高考科技情境里要抓“可控性”，不是只按导电强弱贴标签。

</details>

---

<h4 id="diffusion-brownian-temperature">X3-34 扩散、布朗运动与温度微观解释 <span class="status done">done</span></h4>
...
**例题 1：对象判断**（类型：概念判断）
显微镜下看到花粉颗粒不停抖动，这个“布朗运动”的直接观察对象是什么？

<details><summary>点击查看答案与解析</summary>

**答案**：悬浮的花粉颗粒。

**解析**：布朗运动反映液体分子的无规则运动，但显微镜直接看到的是较大的悬浮颗粒在抖动。

**易错提醒**：不能说显微镜直接看到了水分子运动。

</details>

**例题 2：温度影响**（类型：基础计算）
同种液体中，温度从 `290 K` 升到 `350 K`，布朗运动通常怎样变化？

<details><summary>点击查看答案与解析</summary>

**答案**：通常更明显。

**解析**：温度升高表示分子平均动能增大，分子碰撞更剧烈，悬浮颗粒受到的不平衡冲击更明显。

**易错提醒**：温度影响的是大量分子的平均运动强弱。

</details>

**例题 3：扩散实验**（类型：图像/实验）
把一滴墨水滴入热水和冷水中，哪杯水中颜色更快变均匀？

<details><summary>点击查看答案与解析</summary>

**答案**：热水中更快。

**解析**：热水温度高，分子热运动更剧烈，墨水分子和水分子混合得更快。

**易错提醒**：扩散不需要外力搅拌也能发生，搅拌只是加快混合。
...
**答案**：颗粒越小，来自各方向分子碰撞的平均抵消越不充分，瞬时合力相对更明显。

**解析**：大颗粒同时受到大量分子碰撞，方向更容易平均抵消；小颗粒受碰撞数较少，随机性更突出。

**易错提醒**：布朗运动不是颗粒有生命，也不是液体定向流动。

</details>

**例题 5：错解辨析**（类型：错解辨析）
某同学说：“布朗运动说明花粉分子自己在乱跑。”这个说法对吗？

<details><summary>点击查看答案与解析</summary>

**答案**：不对。

**解析**：布朗运动观察到的是花粉小颗粒整体的无规则运动，原因是周围液体分子对它的不平衡碰撞。

**易错提醒**：要区分“颗粒运动”和“分子热运动”。

</details>

---

<h4 id="molecular-speed-distribution">X3-33 分子运动速率分布图像 <span class="status done">done</span></h4>
...
**例题 1：图像判断**（类型：概念判断）
两条分子速率分布曲线中，峰位更靠右的一条通常对应什么温度？

<details><summary>点击查看答案与解析</summary>

**答案**：通常对应较高温度。

**解析**：温度升高时，分子平均动能增大，高速分子比例增加，最概然速率也增大，所以峰位向右移动。

**易错提醒**：高温曲线峰值会降低，不是整条曲线都向上平移。

</details>

**例题 2：面积含义**（类型：基础计算）
若分布曲线已经按比例归一化，曲线下面积为多少？温度升高后面积变吗？

<details><summary>点击查看答案与解析</summary>

**答案**：面积为 `1`，温度升高后仍为 `1`。

**解析**：归一化曲线下面积表示全部分子的比例。温度改变只改变分布形状，不改变“全部分子总比例”。
...
**例题 3：图像/实验**（类型：图像/实验）
在动画中把温度从 `300 K` 调到 `700 K`，直方图中高速率分子比例怎样变化？

<details><summary>点击查看答案与解析</summary>

**答案**：高速率分子比例增大。

**解析**：升温后平均动能增大，直方图右侧高速率段柱形更明显，分布曲线向右展宽。

**易错提醒**：仍有慢分子存在，不能说所有分子都变成高速分子。

</details>

**例题 4：综合模型**（类型：综合模型）
为什么高温曲线“更矮更宽”，但不表示分子总数变少？

<details><summary>点击查看答案与解析</summary>

**答案**：因为总面积不变，分子比例被分散到更宽的速率范围内。

**解析**：曲线纵坐标表示某速率附近的分子比例密度。温度升高后分布范围变宽，为保持总面积为 1，峰值会降低。

**易错提醒**：纵坐标不是“全部分子数”，要结合面积理解。

</details>

**例题 5：错解辨析**（类型：错解辨析）
某同学说：“温度升高后，所有分子速率都一定变大。”这个说法对吗？

<details><summary>点击查看答案与解析</summary>

**答案**：不对。

**解析**：温度升高表示大量分子的平均动能增大，速率分布整体向高速端扩展，但仍有部分分子速率较小。

**易错提醒**：热学规律描述大量分子的统计平均，不追踪每个分子的确定变化。
